...
for (const scenario of ["resume", "cancel", "new-input"]) {
  test(`real Pi manual compaction: ${scenario}`, { timeout: 15_000 }, async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), "pi-auto-compact-runtime-"));
    let session: any;
    try {
      await mkdir(join(cwd, ".pi"));
      await writeFile(join(cwd, ".pi", "auto-compact.json"), JSON.stringify({
        enabled: true, thresholdPercent: 60,
      }));
      ...
      const model = {
        id: "fake", name: "Offline test", provider: "auto-compact-test", api: "openai-completions",
      ...
      session.subscribe((event: any) => {
        ...
        if (event.type === "agent_settled" && (takeover || manager.getEntries().some((entry: any) => entry.type === "custom_message" && entry.customType === "auto-compact-resume"))) resolveFinished();
      });
      ...
      const resumed = entries.filter((entry: any) => entry.type === "custom_message" && entry.customType === "auto-compact-resume");
      const notices = entries.filter((entry: any) => entry.type === "custom" && entry.customType === "auto-compact-maintenance");
