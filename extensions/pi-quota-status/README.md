...
API 返回 `null` / 缺失字段、或请求失败时，不会出现 `undefined%`、`NaN` 或误导性的 `0%`：

| 场景 | 显示 | 说明 |
| --- | --- | --- |
| 百分比为 `null`（新窗口未计费 / API 未返回）| `5h:--%`（灰色）| 占位符，不会误读成「完全没用」或「用完了」|
| 重置时间缺失或无效 | 省略 `(2h28m)` 后缀 | 不会出现 `NaNhNaNm` |
| 请求超时 | `⚠ ⚡OC 请求超时` | 15s 超时 |
| 断网 / DNS 失败 | `⚠ ⚡OC 网络不可达` | `fetch failed` / `ENOTFOUND` 等归一化 |
| HTTP 401/403 | `⚠ ⚡OC Key 无效或已过期 (HTTP 401)` | |
| HTTP 429 | `⚠ ⚡OC 请求过于频繁 (HTTP 429)` | |
| HTTP 5xx | `⚠ ⚡OC 服务暂不可用 (HTTP 502)` | |

同订阅源瞬时报错时，一分钟内（`STALE_KEEP_MS`）仍显示上次成功数据并附 `?` 后缀标记可能过期；跨订阅源切换则立即显示错误，避免拿旧订阅的数据误导。
