...
export default function (pi: ExtensionAPI): void {
  ...
  function throttled(ctx: StatusCtx, threshold: number) {
    if (
      monitor.matches(ctx.model) &&
      Date.now() - monitor.state.lastRefreshAt < threshold
    )
      return;
    void refresh(ctx);
  }
  ...
  pi.registerCommand("quota", {
    ...
    handler: async (args, ctx: StatusCtx) => {
      ...
      const update = () =>
        action === "account"
          ? account.refresh(ctx.model, () => {}, "openrouterAccount")
          : refresh(ctx);
      if (action === "sources") {
        const adapterId = subscriptionForProvider(ctx.model?.provider);
        ctx.ui.notify(
          quotaDiagnostics(
            monitor.adapter ?? (adapterId ? ADAPTERS[adapterId] : undefined),
            ctx.model,
          ),
          "info",
        );
      ...
      if (!isTui) {
        ctx.ui.notify(
          quotaSummary(target.state, target.adapter, ctx.model),
          "info",
        );
        return;
      }
      while (epoch === generation) {
        const options = [
          "刷新",
          ...(action !== "account" && ctx.model?.provider === "openrouter"
            ? ["查看账户余额（管理 Key）"]
            : []),
          "数据来源与诊断",
          "关闭",
        ...
        if (selected === "查看账户余额（管理 Key）") {
          ...
          await ctx.ui.select(
            quotaSummary(account.state, account.adapter, ctx.model),
            ["返回"],
          );
        } else if (selected === "数据来源与诊断") {
          await ctx.ui.select(quotaDiagnostics(target.adapter, ctx.model), [
            "返回",
          ]);
        } else await update();
      }
