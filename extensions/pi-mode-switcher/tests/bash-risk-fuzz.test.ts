...
const COMPOSITE: Array<
 [name: string, cmd: string, writeExpected: boolean, riskyExpected: boolean]
> = [
 ...
 [
  "interpreter pipe (echo data | python3) — echo 非 provably read-only",
  "echo data | python3 -c 'print(1)'",
  true,
  true,
 ],
 ...
 [
  "touch + cat (write + read composite)",
  "touch newfile && cat newfile",
  true,
  false,
 ],
];
...
describe("bash-risk-fuzz: dangerous commands flagged", () => {
 for (const [name, cmd] of DANGEROUS) {
  it(`isRiskyBash flags: ${name}`, () => {
   assert.equal(isRiskyBash(cmd), true, `expected risky: ${cmd}`);
  });
 }
});
...
describe("bash-risk-fuzz: safe commands pass", () => {
 for (const [name, cmd] of SAFE) {
  it(`isRiskyBash passes: ${name}`, () => {
   assert.equal(isRiskyBash(cmd), false, `expected safe: ${cmd}`);
  });
 }
});
