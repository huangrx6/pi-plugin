...
Model preflight interpretation produces one authoritative relationship, task, intent, risk, domain and coverage result before the normal turn. The default agent source calls the host's active model through `modelRegistry.complete`, using a bounded conversation payload and no tools. The validated result selects the corresponding intent, flow, rigor, profile and domain policies. A host or endpoint failure blocks the live turn; it never silently routes through keyword rules. Deterministic classification is retained only for offline preview and structural safety checks. Uncertainty cannot grant mutation or approval.
...
An interrupted task can be resumed with an exact continuation command such as `继续`, `继续执行`, `continue` or `resume`; a host retry that repeats the original prompt follows the same candidate path. The current model still recognizes the message. When its task, authorization, workflow and model adaptation match the previous decision, the result carries `recognition.policyUnchanged`, the previous policy block is reused, and the event is recorded as `reuse` without a second activity card. A new prompt with additional requirements or a different task shape is classified normally. The provider payload remains independently idempotent: each actual provider request receives one policy block, while no synthetic policy message is added to the session.

Session-aware hosts without a branch API may use a disk fallback. The file name hashes cwd plus session ID. A host without session identity only maintains runtime/session entries and does not use a shared directory-only approval file. Only the current versioned state format is accepted; obsolete directory-only state is ignored.
...
Precedence: package defaults < global config < ancestor-to-nearest project configs < runtime mode/profile/recognition selection. `schema.js` validates types, arrays, enums, integer bounds, nested recognition settings and model rules. `/policy validate` adds policy-reference and file diagnostics. Effective config reports leaf-value sources.

On a runtime configuration error, the previous valid configuration for that cwd is retained and the error is shown. On first load, invalid values use normalized safe defaults, invalid recognition settings cannot initiate a network call, and diagnostics remain visible. Project settings cannot supply credentials, endpoints, history paths or global model rules.
