...
const policies = {
  ...
  "concern.production": "生产环境变更约束", "model.minimax-m3": "MiniMax 模型指令适配", "model.deepseek": "DeepSeek 模型指令适配",
};

export function activitySnapshot(decision, phase, injected = "") {
  const data = structuredClone(decision ?? { rigor: "off" });
  const next = !injected ? "本轮没有追加策略指令。" : phase === "planning"
...
export function activityText(activity) {
  ...
  const applied = activity.injected ? (d.loadedPolicies ?? []).map(id => `  · ${policies[id] ?? id}`) : [];
  return sanitizeTerminalText([activity.summary,
    `为什么：识别为${tasks[d.taskType] ?? d.taskType ?? "未分类"}；风险 ${d.risk ?? "未知"}。`,
    ...(d.reasons?.length ? ["触发依据：", ...d.reasons.map(reason => `  ${reason}`)] : []),
    "实际追加的要求：", ...(applied.length ? applied : ["  无"]),
    ...(d.truncatedPolicies?.length ? [`预算不足，未注入：${d.truncatedPolicies.join("、")}`] : []),
    ...(d.missingPolicies?.length ? [`未找到：${d.missingPolicies.join("、")}`] : []),
    `当时安排：${activity.next}`, "这些是已提供给模型的指令；执行结果需另行验证。", "原文：/policy injected",
  ].join("\n"));
}

export function activityRows(activity, expanded, width) {
  const fallback = { decision: {}, summary: "策略记录不可用", injected: "", next: "无可用记录。" };
  const safe = activity?.decision && typeof activity?.summary === "string" ? activity : fallback;
  const text = expanded ? activityText(safe) : `${safe.summary} · /policy 查看说明`;
  const logicalRows = text.split("\n");
  return logicalRows.flatMap((line, index) => {
    const tone = index === 0
      ? (safe.injected ? "accent" : "muted")
      : /^(预算不足，未注入|未找到)/.test(line)
        ? "warning"
        : /^(触发依据|实际追加的要求)：/.test(line) || /^为什么：|^当时安排：/.test(line)
          ? "text"
          : "dim";
...
export function publishActivity(pi, state, ctx, injected, decision = state.lastDecision) {
  ...
  if (!changed) return;
  try {
    if (typeof pi.appendEntry === "function") pi.appendEntry(ACTIVITY_TYPE, snapshot);
    if (typeof pi.appendEntry !== "function" || typeof pi.registerEntryRenderer !== "function") ctx?.ui?.notify?.(sanitizeTerminalText(`${snapshot.summary} · /policy why 查看说明`), "info");
  } catch {
...
export function phaseText(phase) {
  return { idle: "当前轮已结束，等待新输入。", planning: "正在生成计划。", awaiting_approval: "等待你确认计划；可以批准、提问或修改约束。", executing: "当前任务执行中。" }[phase] ?? "尚未开始。";
}
...
export function restoreActivity(entries) {
  ...
  for (let i = branch.length - 1; i >= 0; i--) {
    ...
    if (entry?.type === "custom" && entry.customType === ACTIVITY_TYPE &&
        data?.decision && typeof data.summary === "string" && typeof data.injected === "string" &&
        typeof data.next === "string" && typeof data.fingerprint === "string") return deepFreeze(structuredClone(data));
  }
  return null;
}
