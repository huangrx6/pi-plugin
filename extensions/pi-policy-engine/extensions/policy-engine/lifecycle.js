...
import { isApprovalPrompt, isPlanRevisionPrompt, shouldBlockTool } from "../../src/core/guard.js";
...
export function registerLifecycleHandlers(pi, { packageRoot, getState }) {
  pi.on("session_start", async (_event, ctx) => {
    ...
    const cfg = buildEffectiveConfig({ packageRoot, cwd: ctx?.cwd ?? process.cwd(), state });
    if (cfg.showStatus !== false) setStatus(ctx, `policy:${cfg.mode ?? "auto"}`);
  });
  ...
  pi.on("model_select", async (event, ctx) => {
    ...
    const cfg = buildEffectiveConfig({ packageRoot, cwd: ctx?.cwd ?? process.cwd(), state });
    if (cfg.showStatus !== false) setStatus(ctx, `policy:${cfg.mode ?? "auto"} · ${modelKey(state.currentModel)}`);
  });
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    if (state.pendingApproval && isApprovalPrompt(prompt) && state.lastDecision) {
      state.pendingApproval = false;
      state.phase = "executing";
      const config = buildEffectiveConfig({ packageRoot, cwd, state });
      const decision = { ...state.lastDecision, gate: config.gate ?? state.lastDecision.gate };
      state.lastDecision = decision;
      const { policies, truncated } = composePolicies({ packageRoot, decision, config, phase: "executing" });
      ...
      const block = renderPolicyBlock({ decision, policies, projectPolicies, phase: "executing", truncated });
      if (config.showStatus !== false) setStatus(ctx, `policy:${decision.workflow}/execute`);
    ...
    if (state.pendingApproval && state.lastDecision) {
      ...
      const { policies, truncated } = composePolicies({ packageRoot, decision, config, phase: "planning" });
      const projectPolicies = loadProjectPolicies(cwd, config);
      decision.loadedPolicies = [...policies, ...projectPolicies].map((p) => p.id);
      decision.truncatedPolicies = truncated;
      const block = renderPolicyBlock({ decision, policies, projectPolicies, phase: "planning", truncated });
      ...
      if (config.showStatus !== false) setStatus(ctx, `policy:${decision.workflow}/planning`);
      return { systemPrompt: `${event.systemPrompt}\n\n${block}\n\n## Pending-plan follow-up\n${followUp}` };
    }

    // Brand new task: classify and route.
    const { decision, config } = decide({
      packageRoot,
      cwd,
      prompt,
      state,
    ...
    if (config.showStatus !== false) setStatus(ctx, `policy:${decision.workflow}/${state.phase}`);
    return { systemPrompt: `${event.systemPrompt}\n\n${block}` };
  });

  pi.on("tool_call", async (event, ctx) => {
    const state = getState();
    if (!state.lastDecision || state.lastDecision.workflow !== "strict") return undefined;

    const config = buildEffectiveConfig({ packageRoot, cwd: ctx?.cwd ?? process.cwd(), state });
    const gate = config.gate ?? state.lastDecision.gate ?? "soft";
    const result = shouldBlockTool(event, gate, state.pendingApproval, config.guard);
