...
const MODE_OPTIONS = [
  ...
  { key: "standard", description: "中等：Task Contract → Inspect → Plan → Execute → Verify" },
...
const GATE_OPTIONS = [
  ...
  { key: "soft", description: "拦截直接写文件工具（write / edit / apply_patch ...）" },
  { key: "hard", description: "soft + 拦截 mutating shell（rm / git push / kubectl apply ...）" },
];

const PROFILE_OPTIONS = [
  { key: "auto", description: "按 taskType 自动选（默认）" },
  { key: "coding", description: "通用编码：execution discipline / minimal change / context hygiene" },
  { key: "debugging", description: "debugging 任务 + debug-first workflow" },
  { key: "documentation", description: "文档/注释：execution discipline + minimal change" },
  { key: "architecture", description: "架构/设计：execution discipline + tool discipline" },
  { key: "review", description: "代码审查：context hygiene + review-first workflow" },
  { key: "research", description: "调研：context hygiene + research-first workflow" },
];
...
async function pickOne(ctx, title, options) {
  ...
  try {
    const choice = await ctx.ui.select(title, selectOptionLabel(options), options);
    if (typeof choice === "string") return parseSelectedKey(choice);
    if (choice && typeof choice === "object" && typeof choice.key === "string") {
      return choice.key;
    }
    return null;
  } catch {
  ...
  return async function policyCommand(args, ctx) {
    ...
    if (action === "profile") {
      ...
      if (!profile || !PROFILE_OPTIONS.some((o) => o.key === profile)) {
        notify(ctx, "Usage: /policy profile " + PROFILE_OPTIONS.map((o) => o.key).join("|"), "warning");
        return;
      }
    ...
    if (action === "status") {
      ...
      notify(ctx, formatStatusSummary({
        config: cfg,
        phase: state.phase,
...
async function runInteractiveSelector(state, ctx) {
  const mode = await pickOne(ctx, "Policy mode (current: " + (state.runtimeMode ?? "auto") + ")", MODE_OPTIONS);
  if (mode) {
  ...
  const gate = await pickOne(ctx, "Policy gate (current: " + (state.runtimeGate ?? "soft") + ")", GATE_OPTIONS);
  if (gate) state.runtimeGate = gate;
  const profile = await pickOne(ctx, "Policy profile (current: " + (state.runtimeProfile ?? "auto") + ")", PROFILE_OPTIONS);
  if (profile) state.runtimeProfile = profile;
  notify(ctx, `Policy: mode=${mode ?? "unchanged"}, gate=${gate ?? "unchanged"}, profile=${profile ?? "unchanged"}`, "info");
}
