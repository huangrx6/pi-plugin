...
- 宿主状态统一为面向用户的中文短文本，不再暴露 `policy:*` 或 `config:policy.*` 内部格式。
- 会话恢复、识别、执行结束和命令切换共用同一状态格式；保留最近识别用量。
- 修正运行时版本常量滞后于包版本的问题，诊断与新写入的历史现在报告实际版本。

## 0.42.0 - 2026-09-10

- 状态发布 key 改为扩展自有名称 `pi-policy-engine`，不再编码外部布局类别。
- 删除状态发布处已经失效的分类协议注释；状态内容和生命周期语义保持不变。
...
- footer badge 与诊断面板标题改为 `↑1.5k ↓50`（↑↓ 数值间加
  空格），与活动卡片、/policy usage 汇总的既有格式统一

## 0.39.1

### 面板「返回」修复：回上一级，不再退出整个面板

- 子面板「返回」此前直接结束整个选择流程；现在回到上一级
  （诊断/设置 → 一级；识别负载/识别模型 → 设置）
- 导航契约：返回 = 上一级；Esc = 随处退出整个面板；动作执行
  完（保存/查看/批准/关闭）= 完成后退出
- 一级面板改为导航循环，子面板返回后重新打开一级选项

## 0.39.0

### 面板信息架构：诊断归组，看/做/改分离

- 一级面板 6 固定 + 动态项：查看状态 / 自动 / 谨慎 / 诊断 / 设置 /
  关闭（+ 按需出现的批准计划 / 结束任务）
- 新增「诊断」二级：识别用量汇总（/policy usage）、路由历史
  （最近 10 条）、运行配置（版本/路径/校验结果）、校验配置
  （/policy validate）；标题实时显示最近识别用量与历史轮数
- 「检查配置」从设置移入诊断（更名「运行配置」），设置收窄为
  纯行为调整：识别负载、识别模型
- 归类原则：一级=每轮可能用的动作；诊断=只看不改；设置=改行为
...
- 一级面板从 7+2 项缩到 5 固定 + 动态项：查看状态 / [批准计划]
  / 自动 / 谨慎 / [结束任务] / 设置 / 关闭
- 「识别负载」「识别模型」「检查配置」收进「设置」二级面板
  （标题实时显示当前负载档与识别模型，不用进去就能看到）
- 动作入口与低频设置不再平级堆叠；跳过面板可用直接命令
  （`/policy once quick`、`/policy context standard`、
  `/policy model <id>` 等）
