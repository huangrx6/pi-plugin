...
为 Pi 的编码、排障、审查和调研任务追加适用的执行要求。当前 agent 默认在正常回答中结合完整对话理解意图，插件再依据识别结果选择流程、领域和模型适配。你可以查看判断方式、加载的要求，以及当前是否在等待确认。
...
默认由当前 agent 在正常回答前先完成一次结构化意图识别，再由插件选择对应的意图、流程、风险和领域策略。识别请求只发送当前对话上下文，不提供工具，也不能授予审批权限。用户消息会先进入对话，Pi 自带的 Working 行显示“意图识别中…”，识别结束后恢复 Pi 的默认 Working 文案，再继续正常回答。
