...
任务账本保留用户要求的原文及来源。约束提取只是索引，遗漏提取不会删除原要求；后续纠正要结合要求顺序理解，不会由分类模型自动擦除旧记录。识别负载中的账本按预算分级收缩（full → trim → minimal，保留最新条目并截断），超长任务不会再因账本膨胀而识别失败；降级档位记录在识别诊断 `taskTier` 中。
