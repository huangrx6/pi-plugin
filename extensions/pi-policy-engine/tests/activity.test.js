...
import { ACTIVITY_TYPE, activityRows, activitySnapshot, activityText, publishActivity, restoreActivity } from "../extensions/policy-engine/activity.js";
...
import { displayWidth, sanitizeTerminalText, wrapTerminalText } from "../extensions/policy-engine/terminal.js";

test("activity snapshots describe loaded policies and cannot drift with runtime state", () => {
  const decision = { rigor: "strict", taskType: "coding", risk: "high", loadedPolicies: ["rigor.strict-plan"], reasons: ["risk:high"], truncatedPolicies: ["domain.database"] };
  const snapshot = activitySnapshot(decision, "planning", "exact injected instructions");
...
test("only changed applied instructions produce transcript records", () => {
  const entries = [];
  const state = { lastDecision: { rigor: "quick", taskType: "documentation" }, phase: "executing" };
...
test("activity rows use theme roles for hierarchy and warnings", () => {
  ...
  assert.ok(rows.some(row => row.tone === "warning" && row.text.includes("预算不足")));
...
test("all command notifications strip terminal control sequences", () => {
  const messages = [];
  notify({ ui: { notify: message => messages.push(message) } }, "safe\x1b]2;title\x07\x1b[31m red\x1b[0m");
  assert.equal(messages[0], "safe red");
});

test("interactive settings change one setting at a time", async () => {
  const state = { runtimeMode: null, runtimeProfile: null, onceMode: null, phase: "idle", lastActivity: { summary: "策略\x1b]2;bad\x07", injected: "" } };
  const selections = ["设置 — 调整模式与配置档", "模式 — 当前 auto", "quick — 快速检查、修改并验证"];
  const notifications = [];
  const titles = [];
  const handler = createCommandHandler({ packageRoot: process.cwd(), getState: () => state });
  await handler("", {
    ui: {
  ...
  assert.equal(state.runtimeProfile, null, "changing mode must not force a profile prompt");
...
test("resuming restores the visible branch explanation and ignores malformed records", () => {
  const activity = activitySnapshot({ rigor: "quick", taskType: "documentation" }, "executing", "instructions");
  const entries = [{ type: "custom", customType: ACTIVITY_TYPE, data: activity }, { type: "custom", customType: ACTIVITY_TYPE, data: {} }];
