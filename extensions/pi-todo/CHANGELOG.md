...
- 模型工具正式接受更符合自然表达的 `complete`，与原有 `finish` 等价，避免操作在进入扩展前就被 JSON Schema 拒绝。
- 重复对进行中任务执行 `start`，或对已完成任务执行 `complete` / `finish`，现在会返回成功的无变化结果；不更新时间、不写盘、不增加 revision。
- 模型提示和工具描述统一指向 `start` / `complete`，减少用 `update` 绕过生命周期动作或猜测不存在动作的概率。
...
- 已完成与归档任务收进历史视图；原有直接命令保留，完整命令目录通过 `/todos commands` 打开。
- 默认编辑器状态条最多两行；`/todos display compact|full|hidden` 可在本次会话切换。
- 窄终端按中文、组合字符和 emoji 的实际显示宽度裁切；任务文本中的完整或残缺终端控制序列不会进入列表、详情或工具结果。
- 取消主任务列表、任务动作或兼容命令面板时静默返回；历史视图仅保留历史任务与返回入口。
- 模型在任务边界更新进度，已恢复的中间错误不会阻止任务完成。
