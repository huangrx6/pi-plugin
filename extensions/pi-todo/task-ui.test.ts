...
test("actions are contextual: blocked tasks cannot start and archived tasks restore", () => {
  ...
  assert.ok(taskActions(state, 2).some((row) => /^close/.test(row)));
  assert.ok(taskActions(state, 2).some((row) => /^review/.test(row)));
...
test("tool display fits a narrow terminal and does not emit task-controlled escapes", () => {
  ...
  const call = tool.renderCall(
    { action: "create", subject: "\x1b]9;fake notification\x07很长的任务名称" },
    theme,
  );
  assert.ok(
    call
      ...
      .every(
        (line: string) =>
          ...
          !line.includes("\x1b]") &&
          !line.includes("fake notification"),
      ),
  );
  const result = tool.renderResult(
    { content: [{ text: "▶ #1 非常长的任务名称，用于验证窄窗口显示" }] },
    { expanded: true },
    theme,
  );
  assert.ok(
    result.render(16).every((line: string) => visibleWidth(line) <= 16),
  );
  resetHarness();
});
