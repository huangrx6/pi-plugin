...
**一个窗口完成浏览。** 按 Tab 在「当前 / 已完成 / 已归档 / 全部」之间切换；`/todos ready` 和 `/todos blocked` 可以直接打开相应筛选。按 `/` 搜索名称或 `#编号`，方向键逐项移动，PageUp / PageDown 翻页，Home / End 跳到首尾。列表高度根据终端自动限制，选中项始终留在可视区域。
