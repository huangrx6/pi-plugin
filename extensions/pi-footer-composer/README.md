...
接管 pi 的 footer，把所有信息渲染成**表格布局**：环境、用量统计、上下文占用、模型、以及**每一个扩展发布的状态**，各占一个单元格，用 `│` 分隔，按终端宽度自动分行。
...
```text
~/project (main) • 优化 │ ↑1.2k ↓890 R340 CH45% $0.012 │ 12%/128k │ (zai-coding-cn) glm-5.3 │ ⚡GLM 5h:4%(4h50m) 周:8%(97h40m)? │ policy:standard
```
...
```text
~/project (main) • 优化
↑1.2k ↓890 R340 CH45% $0.012 │ 12%/128k
(zai-coding-cn) glm-5.3 │ ⚡GLM 5h:4% … │ policy:standard
```

- 单元格顺序：**环境**（cwd / 分支 / 会话名）→ **用量**（↑↓RW / CH / $）→ **上下文**（percent/window，>70% 黄、>90% 红）→ **模型**（含 thinking 级别；多 provider 时带 `(provider)` 前缀）→ **扩展状态**（按 key 排序，一格一个）
- 行宽超过终端时，下一个单元格换行；单元格本身超过整行宽度时截断加 `…`
- 总行数上限 4（防止状态繁多的会话把编辑器挤出屏幕），超出部分收进 `…+N` 溢出格
