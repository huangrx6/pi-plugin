...
test("command parser supports quoted paths and an explicit registry", () => {
  ...
  assert.deepEqual(
    parseCommand("hash case.json --registry=registries/prod.json"),
    {
      action: "hash",
      specPath: "case.json",
      registryPath: "registries/prod.json",
    },
  );
  ...
  assert.throws(
    () => parseCommand("run test.json"),
    /validate、hash 或 registry/,
  );
  assert.throws(
    () => parseCommand("validate test.json --registry="),
    /缺少文件路径/,
  );
  assert.throws(
    () => parseCommand("validate test.json --registry"),
  ...
  assert.throws(
    () => parseCommand("validate 'unterminated.json"),
    /引号未闭合/,
  );
  assert.throws(() => parseCommand("registry a.json extra.json"), /未知参数/);
});

function makePi(handlers: {
  ...
  const logs = await captureConsoleLog(() =>
    command.handler(
      ...
      makeContext({
        ...
        ui: {
          notify() {
            throw new Error("notify must not be used without a UI");
          },
        },
      }),
    ),
...
test("notify keeps working when the host presentation throws", async () => {
  const output = await captureConsoleLog(() =>
    notify(
      makeContext({
        ui: {
          notify() {
            throw new Error("presentation layer exploded");
          },
        },
      }),
      "fallback message",
  ...
  const headless = await captureConsoleLog(() =>
    notify(undefined, "no context at all"),
  );
...
test("missing and malformed files report their role and path", async () => {
  ...
  await command.handler(
    "validate ghost.json --registry examples/capability-registry.json",
    ctx,
  );
  assert.match(messages.at(-1)!, /Test Spec 文件不存在：.*ghost\.json/);

  await command.handler(
    "hash examples/document-upload.test-spec.json --registry nowhere/registry.json",
    ctx,
  );
  assert.match(
    messages.at(-1)!,
    /Capability Registry 文件不存在：.*nowhere\/registry\.json/,
  );
  ...
  try {
    ...
    await command.handler(
      `validate examples/document-upload.test-spec.json --registry '${malformed}'`,
      ctx,
    );
    assert.match(
      messages.at(-1)!,
      /Capability Registry 不是合法 JSON：.*malformed\.json/,
    );
  } finally {
...
test("default registry search explains itself when nothing is found", async () => {
  ...
  try {
    ...
    assert.match(
      messages.at(-1)!,
      /Capability Registry 文件不存在：.*capability-registry\.json/,
    );
...
test("default registry discovery walks up from nested directories", () => {
  ...
  try {
    ...
    writeFileSync(
      join(root, ".pi/browser-test/capability-registry.json"),
      "{}",
    );
    const nested = join(root, "specs/deep");
    mkdirSync(nested, { recursive: true });
    assert.equal(
      findDefaultRegistry(nested),
      join(root, ".pi/browser-test/capability-registry.json"),
    );

    const orphan = join(empty, "a/b");
    assert.equal(
      findDefaultRegistry(orphan),
      join(orphan, ".pi/browser-test/capability-registry.json"),
    );
  } finally {
...
test("MissingJsonFileError carries an optional hint", () => {
  const error = new MissingJsonFileError(
    "Test Spec 文件不存在：/tmp/x.json",
    "use --registry",
  );
...
test("registers an agent-callable validation tool", async () => {
  ...
  const valid = await tool.execute(
    "t1",
    {
      path: "examples/document-upload.test-spec.json",
      registry: "examples/capability-registry.json",
    },
    undefined,
    undefined,
    { cwd: extensionRoot },
  ...
  assert.equal(
    valid.details?.hash,
    "sha256:ccb4dfbeb8cc338c7facd2173c214f3657e6c07a3d49998c25534cbb9b2eb6c3",
  );
  ...
  try {
    const example = JSON.parse(
      readFileSync(
        resolve(extensionRoot, "examples/document-upload.test-spec.json"),
        "utf8",
      ),
    );
    example.revision = 0;
    ...
    const invalid = await tool.execute(
      ...
      {
        path: spec,
        registry: resolve(extensionRoot, "examples/capability-registry.json"),
      },
      undefined,
      undefined,
      { cwd: scratch },
    ...
    const issues = invalid.details?.issues as Array<{
      code: string;
      path: string;
      severity: string;
      message: string;
    ...
    assert.ok(
      Array.isArray(issues) &&
        issues.some(
          (entry) => entry.code === "SCHEMA" && entry.path === "/revision",
        ),
    );
    ...
    const missing = await tool.execute(
      "t3",
      { path: spec, registry: join(scratch, "ghost-registry.json") },
      undefined,
      undefined,
      { cwd: scratch },
    );
  ...
  const empty = await tool.execute("t4", {}, undefined, undefined, {
    cwd: extensionRoot,
  });
...
test("tool batch runs expose per-file structured reports", async () => {
  ...
  try {
    ...
    cpSync(
      resolve(extensionRoot, "examples/fixtures"),
      join(scratch, "fixtures"),
      { recursive: true },
    );
    ...
    broken.revision = 0;
    writeFileSync(
      join(scratch, "broken.test-spec.json"),
    ...
    const okFile = files.find((entry) =>
      entry.file.endsWith("ok.test-spec.json"),
    )!;
    assert.equal(okFile.ok, true);
    assert.match(okFile.hash!, /^sha256:[0-9a-f]{64}$/);
    const brokenFile = files.find((entry) =>
      entry.file.endsWith("broken.test-spec.json"),
    )!;
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        contracts: [
          {
            capabilityId: "cap_x",
            contractVersionId: "capcontract_x_1_0",
            kind: "TRIGGER",
            sideEffect: "NONE",
            externalEffect: "NONE",
...
test("directory arguments validate every sibling test spec", async () => {
  ...
  try {
    ...
    writeFileSync(
      join(scratch, "valid.test-spec.json"),
      JSON.stringify(example),
    );
    const brokenDigest = structuredClone(example);
    brokenDigest.fixtures[0]!.digest.value = "0".repeat(64);
    writeFileSync(
      join(scratch, "broken-digest.test-spec.json"),
      JSON.stringify(brokenDigest),
    );
    writeFileSync(
      join(scratch, "malformed.test-spec.json"),
      "{ schemaVersion: ",
    );
    writeFileSync(join(scratch, "unrelated.txt"), "not a spec");
    mkdirSync(join(scratch, "nested"));
    writeFileSync(
      join(scratch, "nested/ignored.test-spec.json"),
      JSON.stringify(example),
    );

    const registry = resolve(
      extensionRoot,
      "examples/capability-registry.json",
    );
    const messages: string[] = [];
    const ctx = makeContext({
      cwd: scratch,
    ...
    await command.handler(
      `validate '${scratch}' --registry '${registry}'`,
      ctx,
    );
    ...
    try {
      await command.handler(
        `validate '${empty}' --registry '${registry}'`,
        ctx,
      );
